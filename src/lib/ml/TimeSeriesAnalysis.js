...
export class TimeSeriesAnalysis {
    ...
    // Predict next value using time series analysis
    predictNextValue() {
        if (this.data.length < 2) {
            return null;
        }

        // Get recent data points (last 90 days)
        const recentData = this.data.slice(-90);
        if (recentData.length < 2) {
            return this.data[this.data.length - 1].value;
        }

        // Calculate trend using weighted moving average
        const wma = this.calculateSeasonalWMA(7);
        if (wma === null) {
            return this.data[this.data.length - 1].value;
        }

        // Get current season and its patterns
        const currentSeason = this.getTropicalSeason(new Date());
        const seasonalPattern = this.seasonalPatterns[currentSeason];

        // Apply seasonal adjustment
        const seasonallyAdjustedValue = wma * seasonalPattern.weight;

        // Add small random variation (±2%)
        const randomFactor = 1 + (Math.random() * 0.04 - 0.02);

        // Ensure prediction stays within seasonal optimal range
        const prediction = seasonallyAdjustedValue * randomFactor;
        return Math.max(
            seasonalPattern.optimal.min,
            Math.min(seasonalPattern.optimal.max, prediction)
        );
    }
    ...
    predictNextSeasonalValue() {
        ...
        const currentSeason = this.getTropicalSeason(currentDate);
        
        // Get seasonal stats
        const stats = this.getSeasonalYieldStats();
        const seasonalStats = stats[currentSeason];

        if (seasonalStats.count === 0) {
            // If no data for current season, use overall average
            const overallAvg = Object.values(stats).reduce((sum, s) => sum + s.average, 0) / 3;
            return overallAvg * this.seasonalPatterns[currentSeason].weight;
        }

        // Calculate base prediction from seasonal average
        let basePrediction = seasonalStats.average;

        // Apply seasonal weight
        basePrediction *= this.seasonalPatterns[currentSeason].weight;

        // Apply growth trend
        const growthRate = this.calculateBaseGrowthRate();
        basePrediction *= (1 + growthRate);

        // Add small random variation (±3%)
        const randomFactor = 1 + (Math.random() * 0.06 - 0.03);
        const prediction = basePrediction * randomFactor;

        // Ensure prediction stays within seasonal optimal range
        const { optimal } = this.seasonalPatterns[currentSeason];
        return Math.max(optimal.min, Math.min(optimal.max, prediction));
    }
} 