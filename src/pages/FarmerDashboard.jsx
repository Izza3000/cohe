...
const FarmerDashboard = () => {
  ...
  // Move fetchFarmerDashboardData outside of useEffect
  const fetchFarmerDashboardData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (farmerDetailError && farmerDetailError.code !== 'PGRST116') {
          throw farmerDetailError;
        }
        ...
        const { data: harvestData, error: harvestsError } = await supabase
          .from("harvest_data")
          .select("coffee_raw_quantity, coffee_dry_quantity, coffee_premium_grade, coffee_fine_grade, coffee_commercial_grade, harvest_date")
          .eq("farmer_id", authUser.id);
        ...
        // Calculate total Kg for each grade
        ...
        // Calculate total graded yield and percentages
        ...
        setGradePieChartData({
          ...
          datasets: [{
            label: 'Grade Distribution (%)',
            data: [
              Math.round(premiumPercentage * 10) / 10,
        ...
        const sortedRecentHarvests = [...harvestData]
  ...
  const handleRefresh = async () => {
    ...
    try {
      await fetchFarmerDashboardData();
    } catch (err) {
